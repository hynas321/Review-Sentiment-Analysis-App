...
class Menu:
    ...
    def __init__(self):
        self.buttons = (
            ...
            f"{ConsoleColor.YELLOW}1. Add a blob{ConsoleColor.END} input: file_name, blob_name",
            f"{ConsoleColor.YELLOW}2. Display all blobs{ConsoleColor.END} displays: blob_name",
            f"{ConsoleColor.YELLOW}3. Remove a blob{ConsoleColor.END} input: blob_name",
            f"{ConsoleColor.VIOLET}* Vertex AI operations (datasets){ConsoleColor.END}",
            f"{ConsoleColor.YELLOW}4. Create and fill the dataset{ConsoleColor.END} input: dataset_name, blob_name",
            f"{ConsoleColor.YELLOW}5. Display all datasets{ConsoleColor.END} displays: dataset_name, dataset_id etc.",
            f"{ConsoleColor.YELLOW}6. Remove a dataset{ConsoleColor.END} input: dataset_id",
